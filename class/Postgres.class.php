...
class Postgres {
	...
	function Postgres($DB="", $Host="localhost", $PgPort=5432, $User="Anonymous", $pass="Anonymous", $persist=0){
		$this->host = $Host;
		$this->dbname = $DB;
		$this->username = $User;
		$this->password = $pass;
	...
	function Connect(){
		...
		$this->dbconnect = ($this->persistent ? pg_pconnect($connect) : pg_connect($connect));
		if(!$this->dbconnect)
	...
	function FetchResult(&$row=NULL, $assoc=PGSQL_ASSOC){
		if(!$this->error){
...
class Query {
	...
	function Fetch(&$row, $assoc=PGSQL_ASSOC){
		if($this->result)
			$arr = pg_fetch_array($this->result, $row, $assoc);
		else
			$arr = 0;
